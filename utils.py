...
def print_orderbook(bids, asks, bot_order=None):
    ...
    # Adiciona a ordem simulada, se houver
    if bot_order:
...
def log_transaction(side, price, order):
    with open('transacoes.txt', 'a') as f:
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        f.write(f"{timestamp} - Tipo: {side.capitalize()}, Preço: R${price:.2f}, Quantidade: {order['amount']:.4f} BTC\n")
